...
SQLC_DST := $(patsubst %.sql,./internal/sqlc/%.sql.go,$(SQLC_SRC))
...
.PHONY: test/cover
test/cover:
	go test -v -race -buildvcs -coverprofile=${TMPDIR}/coverage.out ./...
...
build: templates sqlc
	...
	go build -o=${TMPDIR}/bin/${BINARY_NAME} ${MAIN_PACKAGE_PATH}
...
run/live:
	go run github.com/air-verse/air@v1.52.3 \
		--build.cmd "make build -j 4" --build.bin "${TMPDIR}/bin/${BINARY_NAME}" --build.delay "100" \
		--build.include_ext "go, html, css, js, templ, sql, jpeg, jpg, gif, png, bmp, svg, webp, ico, env" \
		--build.exclude_regex '_templ.go' \
		--build.exclude_dir 'internal/sqlc' \
